...
dat_fosscal <- "bears_fosscal"
calib_nodes_fosscal <- read_calib_f( main_dir = paste( home_dir, "calib_files/", sep = "" ),
                                     f_names = "Calibnodes_cal1.csv",
                                     dat = dat_fosscal,
                                     head_avail = TRUE )
dat_seccal <- "bears_seccal"
calib_nodes_seccal <- read_calib_f( main_dir = paste( home_dir, "calib_files/", sep = "" ),
                                    f_names = "Calibnodes_cal2.csv",
                                    dat = dat_seccal,
                                    head_avail = TRUE )
...
prior_sum_fosscal <- sum_MCMC( num_dirs = num_chains, delcol = delcol_prior,
                               data_dir = path_prior_fosscal,
                               num_divt = num_divt,
                               node_calib = calib_nodes_fosscal,
                               dataset = "fosscal",
                               ...
                               # could be changed!
                               out_dat = path_prior_fosscal,
                               time_unit = 100 # time unit was scaled by 100
                               # so that time unit was 100 Ma
)
half_chains <- num_chains/2
prior_half1 <- apply( X = prior_sum_fosscal$mean[1:half_chains,],
                      MARGIN = 2, FUN = mean )
prior_half2 <- apply( X = prior_sum_fosscal$mean[(half_chains+1):num_chains,],
                      MARGIN = 2, FUN = mean )
pdf( paste( outchecks_dir,
            "ESS_and_chains_convergence/Convergence_plot_prior_fosscal.pdf",
            sep = "" ),
     paper = "a4" )
plot_convergence( name_dir_dat = "Prior, fosscal",
...
th <- 0.1
sum_quantiles <- check_quantiles( dat = prior_sum_fosscal, threshold = th,
...
check_quantiles( dat = prior_sum_fosscal, threshold = th, main_chain = main_ch,
                 num_chains = num_chains, compare_all = FALSE,
                 outdir = path_prior_fosscal )
...
ESS_fosscal <- sum_MCMC_ESS( x = prior_sum_fosscal$arr4stan,
                             samp_per_chain = prior_sum_fosscal$samp_per_chain )
# Minimum samples per chain
min( prior_sum_fosscal$samp_per_chain  )    
# Median samples per chain
median( prior_sum_fosscal$samp_per_chain  ) 
# Maximum samples per chain
max( prior_sum_fosscal$samp_per_chain  )    
# Number of samples used to compute the ESS
min( prior_sum_fosscal$samp_per_chain  ) * num_chains
# Show tail-ESS and bulk-ESS
ESS_fosscal$tab
## Calculate Rhat, min and max. Good if max Rhat <= 1.05
min( ESS_fosscal$stats$Rhat ) 
max( ESS_fosscal$stats$Rhat )
# Number of samples collected throughout
dim( prior_sum_fosscal$all_mcmc )


#### ANALYSES SECCAL ####

# 1. Obtain summarise object and generate first convergence plot
prior_sum_seccal <- sum_MCMC( num_dirs = num_chains, delcol = delcol_prior,
                               data_dir = path_prior_seccal,
                               num_divt = num_divt,
                               node_calib = calib_nodes_seccal,
                               dataset = "seccal",
                               perc = perc, def_samples = def_samples,
                               prior = TRUE,
                               # In this case, I want the output dir to be
                               # created within the same data dir, but it
                               # could be changed!
                               out_dat = path_prior_seccal,
                               time_unit = 100 # time unit was scaled by 100
                               # so that time unit was 100 Ma
)
half_chains <- num_chains/2
prior_half1 <- apply( X = prior_sum_seccal$mean[1:half_chains,],
                      MARGIN = 2, FUN = mean )
prior_half2 <- apply( X = prior_sum_seccal$mean[(half_chains+1):num_chains,],
                      MARGIN = 2, FUN = mean )
pdf( paste( outchecks_dir,
            "ESS_and_chains_convergence/Convergence_plot_prior_seccal.pdf",
            sep = "" ),
     paper = "a4" )
plot_convergence( name_dir_dat = "Prior, seccal",
                  mean_divt1 = prior_half1,
                  mean_divt2 = prior_half2,
                  num_runs = num_chains )
dev.off()
...
#> 1. Set threshold and run preliminary comparison anlaysis across MCMC runs
th <- 0.1
sum_quantiles <- check_quantiles( dat = prior_sum_seccal, threshold = th,
                                  num_chains = num_chains, compare_all = TRUE )
#> 2. Find out which is the one that should be used as "main chain" against
#> which the rest should be compared (i.e., the one with less differences in
#> q97.5% and q2.5%)
...
#> 3. Find out which chains should be removed by using this main chain
check_quantiles( dat = prior_sum_seccal, threshold = th, main_chain = main_ch,
                 num_chains = num_chains, compare_all = FALSE,
                 outdir = path_prior_seccal )
#> END CHECK: no problems, we can proceed! If there were any problematic chains,
#> you would need to re-run the commands above but after getting rid of 
#> the problematic chain/s
...
##> posterior) -- more conservative, but only way the function can be used
##> properly to my knowledge.
ESS_seccal <- sum_MCMC_ESS( x = prior_sum_seccal$arr4stan,
                             samp_per_chain = prior_sum_seccal$samp_per_chain )
# Minimum samples per chain
min( prior_sum_seccal$samp_per_chain  )    
# Median samples per chain
median( prior_sum_seccal$samp_per_chain  ) 
# Maximum samples per chain
max( prior_sum_seccal$samp_per_chain  )    
# Number of samples used to compute the ESS
min( prior_sum_seccal$samp_per_chain  ) * num_chains
# Show tail-ESS and bulk-ESS
ESS_seccal$tab
## Calculate Rhat, min and max. Good if max Rhat <= 1.05
min( ESS_seccal$stats$Rhat ) 
max( ESS_seccal$stats$Rhat )
# Number of samples collected throughout
dim( prior_sum_seccal$all_mcmc )
