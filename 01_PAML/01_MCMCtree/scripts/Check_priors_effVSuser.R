...
# In this case, you are using the concatenated file, and so the number of
# samples is 100005, and so you need to specify one less
def_samples <- 100004
...
# approaches.
num_dirs    <- 2
path_prior  <- c( paste( home_dir, "sum_analyses/prior/mcmc_files_fosscal", sep = "" ),
                  paste( home_dir, "sum_analyses/prior/mcmc_files_seccal", sep = "" ) )
...
calib_nodes <- read_calib_f( main_dir = paste( home_dir, "calib_files/", sep = "" ),
                             f_names = c( "Calibnodes_cal1.csv", "Calibnodes_cal2.csv" ),
                             dat = dat,
                             head_avail = TRUE )
...
names( mcmc_priors ) <- c( "fosscal", "seccal" )
count <- 0
